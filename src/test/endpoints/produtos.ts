...
describe("Atualizar um produto", () => {
  it("Deve atualizar um produto", async () => {
    ...
    const response = await request(app)
      .put("/produtos")
      .set("Authorization", "Bearer " + process.env.TOKEN_TEST)
      .send({ id, nome: "Produto de teste", preco: 10.5 });
    ...
    expect(response2.body.nome).toBe("Produto de teste");
    expect(response2.body.preco).toBe(10.5);
  });
});
